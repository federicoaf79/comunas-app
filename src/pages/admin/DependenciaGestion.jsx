...
function TabInformacion({ dep, dependenciaId }) {
  ...
  async function handleSave() {
    ...
    try {
      // .select() al final del update: un 204 sin filas no permite
      // distinguir "guardó" de "no encontró la fila" — con .select(),
      // si vuelve vacío el guardado tira error explícito en vez de
      // mostrar "Datos guardados" sin haber confirmado nada.
      let { data: rows, error } = await supabase
        .from('dependencias').update(patchFull).eq('id', dependenciaId).select(DEP_COLS_FULL)
      if (error && /column .* does not exist|42703/i.test(error.message ?? '')) {
        // `responsable` no existe en esta instancia — reintento sin él.
        const { responsable: _omit, ...patchBase } = patchFull
        ;({ data: rows, error } = await supabase
          .from('dependencias').update(patchBase).eq('id', dependenciaId).select(DEP_COLS_BASE))
      }
      if (error) throw error
...
// ─────────────────────────────────────────────────────────────────
// TAB 6 · Bot IA — vive en components/admin/DepBotIATab.jsx.
// Antes había una copia entera acá (duplicado real, no solo el
// mismo nombre): mismos 3 campos, mismo endpoint de sync, pero con
// el mismo `dep` liviano de fetchDependencia() de más arriba, que
// nunca trajo bot_descripcion/bot_faq/bot_restricciones. Arreglar
// DepBotIATab no arreglaba esta copia — de ahí que las ~15
// dependencias que pasan por esta pantalla genérica siguieran rotas
// después del fix original. Se elimina la copia y se usa el
// componente compartido, que ya resuelve su propia fila por id y
// confirma el guardado con .select().
// ─────────────────────────────────────────────────────────────────
  ...
  return (
    <div className="space-y-5">
      ...
      <div key={tab} className="animate-fade-in">
        ...
        {tab === 'bot_ia'    && dep && <DepBotIATab dep={dep} />}
