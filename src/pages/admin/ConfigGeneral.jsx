...
function BotDependenciasForm({ municipioId, disabled }) {
  ...
  useEffect(() => {
    ...
    supabase
      .from('dependencias')
      .select('id, nombre, tipo, bot_descripcion, bot_faq, bot_restricciones')
      .eq('municipio_id', municipioId)
      .eq('activa', true)
      .order('nombre')
      .then(({ data, error }) => {
        if (!error && data) setDeps(data.map(d => ({ ...d })))
        setLoading(false)
      })
  }, [municipioId])
  ...
  async function handleGuardar() {
    ...
    setGuardado(false)
    try {
      // 1. Guardar en Supabase cada dependencia
      for (const dep of deps) {
        await supabase.from('dependencias').update({
          bot_descripcion:   dep.bot_descripcion   || null,
          bot_faq:           dep.bot_faq           || null,
          bot_restricciones: dep.bot_restricciones || null,
        }).eq('id', dep.id)
      }

      // 2. Sincronizar con Plan-B
      const syncRes = await fetch('/api/sync-planb', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!syncRes.ok) {
        ...
        throw new Error(err.error || 'Error al sincronizar con Plan-B')
      }
  ...
  return (
    <SectionShell
      title="Bot IA — Configuración por dependencia"
      desc="Definí la información específica que el bot conoce de cada dependencia. Se sincroniza automáticamente con Plan-B."
      error={error}
      ok={guardado ? 'Guardado y sincronizado con Plan-B correctamente.' : ''}
    >
      {loading ? (
        <div className="flex justify-center py-8"><Spinner /></div>
      ) : deps.length === 0 ? (
        <p className="text-sm text-primary-400">No hay dependencias activas configuradas.</p>
      ) : (
        <div className="space-y-6">
          {deps.map(dep => (
            <div key={dep.id} className="rounded-lg border border-border p-4">
              <h3 className="mb-3 font-sora text-sm font-bold text-primary">{dep.nombre}</h3>
              <div className="space-y-3">
                <div>
                  <label className="mb-1 block text-xs font-semibold text-primary-500 uppercase tracking-wide">
                    Descripción para el bot
                  </label>
                  <textarea
                    rows={3}
                    disabled={disabled}
                    value={dep.bot_descripcion || ''}
                    onChange={e => updateDep(dep.id, 'bot_descripcion', e.target.value)}
                    placeholder={`Descripción de ${dep.nombre} para que el bot responda preguntas generales...`}
                    className="w-full rounded-md border border-border bg-white px-3 py-2 text-sm text-primary placeholder:text-primary-300 focus:outline-none focus:ring-2 focus:ring-accent disabled:opacity-50"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-semibold text-primary-500 uppercase tracking-wide">
                  ...
                  <textarea
                    ...
                    disabled={disabled}
                    value={dep.bot_faq || ''}
                    onChange={e => updateDep(dep.id, 'bot_faq', e.target.value)}
                    placeholder={"**¿Cuándo atienden?** Lunes a viernes 8-13hs\n**¿Necesito turno?** Sí, pedilo por este chat"}
                    className="w-full rounded-md border border-border bg-white px-3 py-2 text-sm text-primary placeholder:text-primary-300 focus:outline-none focus:ring-2 focus:ring-accent disabled:opacity-50 font-mono text-xs"
                  />
                  <p className="mt-1 text-[11px] text-primary-400">Usá formato **Pregunta** → Respuesta</p>
                </div>
                <div>
                  <label className="mb-1 block text-xs font-semibold text-primary-500 uppercase tracking-wide">
                  ...
                  <textarea
                    rows={2}
                    disabled={disabled}
                    value={dep.bot_restricciones || ''}
                    onChange={e => updateDep(dep.id, 'bot_restricciones', e.target.value)}
          ...
          <div className="flex justify-end pt-2">
            ...
            >
              ...
              {saving ? 'Sincronizando...' : 'Guardar y sincronizar con Plan-B'}
            </button>
          </div>
        </div>
      )}
...
function BotDependenciasSection({ disabled, municipioId }) {
  return <BotDependenciasForm municipioId={municipioId} disabled={disabled} />
}
    ...
    <div className="space-y-5">
      ...
      {!esperandoFallback && !sinMunicipio && (
        <div className="space-y-5">
          ...
          <BotDependenciasSection   disabled={sinMunicipio} municipioId={municipioId} />
