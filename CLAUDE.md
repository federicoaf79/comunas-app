...
- **`mockData.js` NO quedó huérfano** — lo siguen usando `CrmVecinos.jsx`/`VecinoFormModal.jsx`/`HistoriaClinicaForm.jsx` (catálogo estático de `barrios`) y `VecinoDetail.jsx` (`mensajesByVecino`, la pestaña "Mensajes" de la ficha de un vecino puntual — comentario propio en ese archivo ya decía "sigue con mock por ahora, se conecta en una iteración posterior"; deliberadamente fuera de alcance de este fix, es un problema distinto: mock por vecino individual, no un historial global compartido por todo el municipio).
- **Pendiente de decidir (no se tocó):** `mensajeria` sigue afuera de `MODULOS_DEFAULT_ON` (ver entrada de arriba) porque la razón original de excluirlo — datos falsos haciéndose pasar por reales — ya no aplica. Podría volver a entrar al default del wizard ahora que el historial es honesto (vacío real en vez de mentira), aunque el envío en sí siga sin conectar. Confirmar con el cliente antes de cambiar `MODULOS_DEFAULT_ON`.
