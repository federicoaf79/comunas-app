...
**MEDIO — Agenda pública vacía para julio 2026 en adelante (data, no código):** confirmado 2026-07-23 (auditoría de UX) que los 10 eventos de `agenda_publica` para Real Sayana tienen vigencia vencida — 6 son recurrentes (`recurrente=true`) con `fecha_inicio=2026-06-01`/`fecha_fin=2026-06-30`, los otros 4 son puntuales de fechas específicas ya pasadas (19/6, 21/6, 25/6, 28/6). Resultado: tanto `/admin/agenda-publica` como el portal público (`/portal/agenda`) muestran la semana actual completamente vacía. No es un bug de código — es data de seed que expiró y nadie extendió. Fix pendiente: correr un `UPDATE agenda_publica SET fecha_fin = '2026-12-31' WHERE municipio_id = '654d0e86-255d-4498-b5c9-80d91793d318' AND recurrente = true AND fecha_fin = '2026-06-30'` (afecta los 6 eventos recurrentes; los 4 puntuales se dejan como están a propósito, moverlos sería inventar contenido). SQL entregada al usuario 2026-07-23 para que la corra él mismo — no ejecutada por Claude (regla de seguridad operativa de este archivo).
...
**BAJO (hallazgo lateral, no arreglado) — el modal "Nueva reserva" de SUM no envía el INSERT real:** al intentar verificar `sum_reservas` en vivo, el botón "Guardar reserva" en `/admin/sum` no disparaba ningún POST (ni a `sum_reservas` ni a `audit_log`), con `[useSumReservas] createSumReserva error` en consola. Reproduje el mismo INSERT a mano (mismo `dependencia_id`, `municipio_id`, `solicitante`, `fecha`, `horario`, `estado`) directo contra la REST API y funcionó perfecto (201) — así que `createSumReserva()` y su wiring de auditoría están bien; el bug está en cómo el modal arma o dispara el payload del formulario (no investigado a fondo, no es del sprint de auditoría). Pendiente para quien toque `Sum.jsx` después.
