...
**ALTO — `mensajeria` (Mensajería SMS/WhatsApp) muestra historial FALSO, no datos vacíos — Real Sayana lo tiene activo HOY:** confirmado 2026-07-30 al decidir si `mensajeria` entraba en el default-on del punto anterior. `Mensajeria.jsx` no lee `sms_log` (la tabla real existe, vacía) — importa `mensajes`/`vecinoById` de `lib/mockData.js`. El panel "Historial" muestra 5 mensajes 100% hardcodeados (`vecino_id: v1/v3/v5/v7/v8`, sin relación con ningún vecino real de ningún tenant) como si fueran envíos reales del municipio. El botón "Enviar" (`handleSend`) hace `alert('Envío simulado...')` sin llamar ninguna API — no es que falte Twilio y falle: directamente no intenta enviar nada real, así que tampoco rompe por falta de credenciales. El problema no es que sea mock — es que **el staff de un municipio real puede ver "historial" de mensajes que nadie mandó, a vecinos que no son los suyos**, en un sistema de gestión municipal en producción. Como Real Sayana ya tiene `mensajeria` activo hoy (era parte del preseleccionado original del wizard, antes de este fix), este historial fantasma ya es visible para su staff ahora mismo, no es un riesgo hipotético de tenants futuros. **Pendiente, va a la limpieza pre-entrega:** conectar `Mensajeria.jsx` a `sms_log` real (y a vecinos reales para el segmento), o como mínimo mostrar un estado vacío honesto ("Todavía no se enviaron mensajes") en vez de `mockData.js`. Hasta que eso pase, `mensajeria` queda fuera del preseleccionado del wizard para tenants nuevos (ver `MODULOS_DEFAULT_ON`, `useModulos.js`).
