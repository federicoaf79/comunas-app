...
**ALTO (hallazgo real 2026-07-23, descubierto al reverificar con sesión limpia) — `ordenes_derivacion.validada_por` viola su FK en cualquier subida real de un vecino:** con la sesión ya consistente, el INSERT a `ordenes_derivacion` pasó la RLS pero falló con `insert or update on table "ordenes_derivacion" violates foreign key constraint "ordenes_derivacion_validada_por_fkey"`. Causa: `useOrdenMedicaUpload.js` manda `validada_por: user?.id` — el `auth.uid()` de quien SUBE el archivo (el vecino) — pero esa columna es FK a `usuarios.id` (solo staff). Un vecino real nunca tiene fila en `usuarios`, así que este INSERT **falla siempre** para cualquier vecino autenticado de verdad, no solo en pruebas contaminadas. Además es un error de diseño independiente del FK: `validada_por` debería quedar `null` hasta que un miembro del staff valide la orden desde `CicSalud.jsx` (que si actualiza `validada_por` correctamente en `validarOrden()`), no completarse con el uploader en el momento de subir. El turno en sí SÍ se crea bien (`pendiente_validacion`), pero la orden física adjunta nunca llega a existir como fila — mismo efecto práctico que el hallazgo anterior (el botón "Validar orden" no tiene nada real para validar), pero por una causa completamente distinta. No tocado — el fix es sacar `validada_por` del payload de `uploadOrden()` (dejar que la columna quede en su default/null hasta la validación real).
