...
**ALTO — `pendiente_validacion` no existe en el CHECK constraint de `turnos_agenda.estado`, y el flujo de orden médica lo usa en dos puntos:** confirmado en vivo 2026-07-23 (`23514 turnos_agenda_estado_check`) que los únicos estados reales aceptados en prod son `confirmado | pendiente | atendido | cancelado`. Esto rompe DOS cosas relacionadas, no una sola:
  1. `validarOrden()` en `CicSalud.jsx` — el botón "Validar orden" solo se renderiza cuando `turno.estado === 'pendiente_validacion'`, código inalcanzable porque ese estado nunca puede existir en una fila.
  2. `SacarTurnoFormPortal.jsx:435` — cuando un vecino pide turno para una especialidad que `requiere_orden` Y no tiene una derivación digital ya validada, el payload arma `estado: 'pendiente_validacion'` → el INSERT real fallaría con el mismo `23514` (no probado en vivo hoy porque el turno de prueba fue con Odontología, que no requiere orden — confirmar con una especialidad que sí la requiera antes de asumir que está roto en producción real).
  No tocado — evaluar si hay que agregar `pendiente_validacion` al constraint (lo más simple, ya que todo el flujo de orden médica física asume ese estado) o si conviene rediseñar sin ese estado intermedio.
