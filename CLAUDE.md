...
**BAJO — `validarOrden()` en `CicSalud.jsx` es código inalcanzable:** el botón "Validar orden" solo se renderiza cuando `turno.estado === 'pendiente_validacion'`, pero ese valor **viola el CHECK constraint de `turnos_agenda.estado`** — nunca puede insertarse. Confirmado en vivo 2026-07-23 (`23514 turnos_agenda_estado_check`) y que los únicos estados reales usados en prod son `confirmado | pendiente | atendido | cancelado`. La función nunca se ejecuta en la práctica. No tocado — evaluar si hay que agregar `pendiente_validacion` al constraint o si el flujo de validación de órdenes físicas se resolvió por otro camino que no usa este estado.
...
**Fases pendientes** (mismo orden acordado): 3 — `turnos_agenda`/`beneficiarios`/`reclamos` (resolver la FK de vecino antes de tocar `turnos_agenda`); 4 — exportaciones CSV (6 puntos: Administración, Rendición, Inventario, Importador Vecinos, Auditoría, Patrimonio); 5 — resto administrativo (dependencias, profesionales, expedientes, inventario, flota, seguros, patrimonio, obras, SUM, agenda pública, autoridades, dominios, historia municipio).
