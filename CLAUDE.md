...
- **PENDIENTE DE VERIFICAR EN VIVO (no se pudo hacer acá):** que `generateLink` efectivamente NO dispare el mail automático de Supabase además del de Resend — el comportamiento documentado de Supabase dice que no, pero requiere una invitación real de prueba con la `RESEND_API_KEY` ya cargada en Vercel para confirmarlo (bandeja de entrada real). Si llega un mail duplicado, hay que desactivar el template de invitación desde el Supabase Dashboard.
