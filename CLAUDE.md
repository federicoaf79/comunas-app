...
**BAJO (hallazgo nuevo, no investigado) — Odontología muestra un bloque de "planilla imprimible" con datos de Sala Primeros Auxilios fuera de `@media print`:** al verificar el fix de arriba, `/admin/dependencia/odontologia` mostraba visible en pantalla (no solo al imprimir) un bloque con clase `planilla-title` mostrando "SALA DE PRIMEROS AUXILIOS" y turnos de esa otra dependencia. `Odontologia.jsx` no importa `PlanillaImprimir` (confirmado por grep), así que no está claro de dónde viene ese bloque ni por qué no respeta `@media print`. No investigado — cosmético, no bloquea nada funcional.
