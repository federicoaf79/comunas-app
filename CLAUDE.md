...
- **Hallazgo sin resolver, marcado a propósito para decidir con el usuario:** `ResetPassword.jsx` redirige después de un cambio de contraseña exitoso a `/portal/acceso` (login de VECINO), nunca a `/login` (login de STAFF). Para el flujo de invitación de USUARIOS (staff), el usuario nuevo termina de crear su contraseña y cae en la puerta equivocada. No se tocó — la página es compartida por cualquier flujo que use `generateLink` (invitación de staff y recuperación de contraseña de vecino por igual) y no hay hoy una señal en la URL/sesión que distinga "vengo de una invitación de staff" de "vengo de un reset de vecino" sin diseñar algo nuevo. Pendiente decidir: ¿un query param en `redirectTo` que la página lea para elegir el destino post-éxito, o una página de aterrizaje separada para el flujo de staff?
- **Contador de "pendientes de activación" en `/admin/usuarios`:** mismo patrón que "Pendientes (N)" en `CrmVecinos.jsx` — banner visible + count en el filtro "Inactivos". `usuarios.activo` no distingue "invitación sin aceptar todavía" de "desactivado a mano" (mismo booleano para las dos cosas, sin un campo separado en el schema) — el texto del banner ("incluye invitaciones sin aceptar todavía") es deliberadamente honesto sobre esa limitación en vez de prometer más precisión de la que el dato tiene.
