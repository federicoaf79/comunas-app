...
**ALTO (hallazgo nuevo 2026-07-23) — RLS bloquea el INSERT en `ordenes_derivacion` desde el upload de orden médica física del vecino:** al reverificar el fix de arriba, el paso siguiente del mismo flujo (`useOrdenMedicaUpload.js` → `uploadOrden()`, llamado por `SacarTurnoFormPortal.jsx` cuando NO hay derivación digital y el vecino sube un archivo) falla en vivo con `new row violates row-level security policy for table "ordenes_derivacion"` — el upload a Storage (`documentos-hc`) sí funciona, pero el INSERT posterior en `ordenes_derivacion` (`origen:'fisica', estado:'pendiente'`) queda bloqueado para el rol del vecino autenticado. Efecto práctico: el turno se crea igual (`pendiente_validacion`, botón "Validar orden" visible para staff), pero **nunca queda ningún registro en `ordenes_derivacion` para validar** — si el staff hace click en "Validar orden", `validarOrden()` no encuentra filas (`turno_id` + `estado='pendiente'`) para actualizar, pero igual fuerza `turnos_agenda.estado → 'confirmado'` sin haber validado nada real. No tocado — falta agregar (o corregir) una policy de INSERT en `ordenes_derivacion` para el vecino autenticado dueño del turno.
