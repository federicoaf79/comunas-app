...
**MEDIO — `ImportadorVecinos.jsx`: detección fuzzy de duplicados sin probar con volumen real, riesgo de congelar la pestaña en la carga del padrón de Real Sayana (día uno):** `detectFuzzyDuplicates()` es `O(n²)` — compara cada par del lote recién importado con Levenshtein completo, en un solo bucle síncrono en el hilo principal, sin ceder el hilo. Con las ~2.000 filas que Real Sayana va a cargar de una sola vez son ~2.000.000 de pares. No probado todavía con un archivo de ese tamaño antes de la entrega. Si congela la pestaña, dos salidas: acotar las comparaciones (ej. solo contra vecinos con la misma inicial de apellido, para bajar drásticamente los pares) o mover el cálculo a un Web Worker. **Pendiente: probarlo con un archivo de ~2.000 filas generadas antes de la entrega — es el único riesgo que queda del importador para el día uno**, el resto (batching de 200, reporte de errores por fila con CSV descargable, dedup real por DNI vía `fetchExistingDnis()` paginada, `skipped` con sentido real) ya se arregló y buildeó 2026-07-29.

De paso, en el reintento fila-por-fila del batch de inserts, falta confirmar que el mensaje de error que llega al CSV sea el real de Postgres y no el de `.select().single()` tapando un DNI duplicado con "JSON object requested, multiple (or no) rows returned" — no verificado en vivo todavía.
